...
<div class="px-5 pt-3">
	<div class="flex gap-3 overflow-x-auto pb-1 snap-x snap-mandatory hide-scrollbar">
		{#each promos as promo, i}
			<div
				class="flex min-w-[240px] shrink-0 snap-start items-center gap-3 rounded-xl {promo.bg} {promo.text} px-4 py-3.5 opacity-0"
				style="animation: popIn 0.5s cubic-bezier(0.22, 1, 0.36, 1) {i * 80}ms forwards;"
			>
				<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg {promo.id === 3 ? 'bg-[#A4161A]/10 text-[#A4161A]' : 'bg-white/15'}">
					<svelte:component this={promo.icon} size="20px" />
				</div>
				<div class="min-w-0 flex-1">
					<p class="text-sm font-bold leading-tight">{promo.title}</p>
					<p class="mt-0.5 text-xs {promo.id === 3 ? 'text-[#6B705C]' : 'text-white/60'} leading-snug">{promo.subtitle}</p>
				</div>
